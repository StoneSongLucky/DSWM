%% Note
%%% synthetic data test with amplitude and horizon noise
clc;
clear;
close all;
%% Load data
...
%% Constract noised 2D horizon for 3D synthetic data
rand('seed', 0);
scaleDeepth = 5;
varDeepth1 = (-1 + 2 * rand(inlineNum*crosslineNum,1));
varDeepth2 = (-1 + 2 * rand(inlineNum*crosslineNum,1));
hori2DTop = repmat(topHorizon,1,crosslineNum)' + reshape(varDeepth1,crosslineNum,inlineNum)*scaleDeepth;
hori2DBottom = repmat(bottomHorizon,1,crosslineNum)' + reshape(varDeepth2,crosslineNum,inlineNum)*scaleDeepth;
%% Show data
showData = squeeze(seiData3DNoise(:,1,:))';
showSeiData(showData(1:3:end,:)); hold on;
plot(hori2DTop(1,1:3:end), 'LineWidth',2); hold on;
plot(hori2DBottom(1,1:3:end), 'LineWidth',2);
%% Label data
